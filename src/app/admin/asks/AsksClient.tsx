...
export default function AdminAsksClient({ asks }: { asks: AskWithRelations[] }) {
  ...
  const send = (id: string, matched: boolean) => {
    ...
    startTransition(async () => {
      await replyToAsk(id, text, matched);
      setDrafts((d) => ({ ...d, [id]: "" }));
    });
  };
          ...
          <div key={a.id} className="card p-5">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`chip !text-[10px] ${statusTone(a.status)}`}>{statusLabel[a.status]}</span>
              </div>
            </div>
            ...
            {a.replies.length > 0 && (
              <div className="mt-3 space-y-1.5">
                {a.replies.map((r) => (
                  <p key={r.id} className="rounded-lg bg-green-soft px-3 py-2 text-xs text-slate">
                    <b className="text-green">Sent {r.createdAt.toLocaleDateString("en-IN")}:</b> {r.text}
                  </p>
                ))}
              </div>
            )}
