    ...
    <div className="space-y-6">
      ...
      {isCp && (
        <div className="card flex flex-wrap items-center justify-between gap-3 border-l-4 p-5" style={{ borderLeftColor: "var(--gold)" }}>
          ...
          <span className="chip badge-gold">{requestCount} customization{requestCount === 1 ? "" : "s"} in queue</span>
        </div>
      )}
          ...
          <div key={c.id} className="card card-hover overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 flex gap-2">
                ...
                {isCp && c.status === "Ready" && (
                  <button
                    onClick={() => startTransition(async () => { await requestCreative(c.title); setRequested(c.id); })}
                    disabled={isPending}
                    className={`btn flex-1 justify-center !py-2 !text-xs text-white disabled:opacity-60 ${requested === c.id ? "!bg-green" : "!bg-ink hover:!bg-green"}`}
                  >
